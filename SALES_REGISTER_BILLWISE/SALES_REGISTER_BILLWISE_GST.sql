alter PROCEDURE [dbo].[SALES_REGISTER_BILLWISE_GST] 
	 @FromDate datetime,
     @ToDate datetime
...
left join (select (SUM(ISNULL(TaxSum, 0)) -sum(ISNULL(RvsChrgTax,0))) 'TaxSum',docentry from inv4 CGST where CGST.Statype=-100 group by docentry) as CGST on CGST.docentry=oi.docentry --reverse charge
left join (select (SUM(ISNULL(TaxSum, 0)) -sum(ISNULL(RvsChrgTax,0))) 'TaxSum',docentry  from inv4 SGST where  SGST.Statype in (-110,-150) group by docentry) as SGST on SGST.docentry=oi.docentry --reverse charge --UGST
left join (select (SUM(ISNULL(TaxSum, 0)) -sum(ISNULL(RvsChrgTax,0))) 'TaxSum',docentry  from inv4 IGST where  IGST.Statype=-120 group by docentry) as IGST on IGST.docentry=oi.docentry --reverse charge
left join (select (SUM(ISNULL(TaxSum, 0)) -sum(ISNULL(RvsChrgTax,0))) 'TaxSum',docentry  from inv4 OTHER where  OTHER.Statype not in (-100,-110,-120,-150) group by docentry) as OTHER on OTHER.docentry=oi.docentry --OTHER AND reverse charge

left outer join(select INV3.docentry, sum(case when INV3.Fixcurr='INR' then isnull((INV3.LineTotal),0) else isnull((INV3.totalsumsy),0) end) as DocLevFreight
from INV3 where INV3.ExpnsCode!='' Group by INV3.docentry ) DocLevFreight  on I1.docentry=DocLevFreight.docentry 
...
from [dbo].[ORIN] oi 
INNER JOIN [dbo].[RIN1] i1  ON  OI.DocEntry = I1.DocEntry and i1.basetype<>203  --09-05-2020  --changes as per reliable if only fright is included not item 14/05/2020
LEFT OUTER JOIN OSTC O ON O.CODE = I1.TAXCODE 
LEFT outer join OITM M1 ON I1.ItemCode=M1.ItemCode 	
left outer JOIN NNM1 N2 ON N2.SERIES = OI.SERIES

left join (select (SUM(ISNULL(TaxSum, 0)) -sum(ISNULL(RvsChrgTax,0))) 'TaxSum',docentry from rin4 CGST where CGST.Statype=-100 group by docentry) as CGST on CGST.docentry=i1.docentry --reverse charge
left join (select (SUM(ISNULL(TaxSum, 0)) -sum(ISNULL(RvsChrgTax,0))) 'TaxSum',docentry  from rin4 SGST where  SGST.Statype in (-110,-150) group by docentry) as SGST on SGST.docentry=i1.docentry --reverse charge --UGST
left join (select (SUM(ISNULL(TaxSum, 0)) -sum(ISNULL(RvsChrgTax,0))) 'TaxSum',docentry  from rin4 IGST where  IGST.Statype=-120 group by docentry) as IGST on IGST.docentry=i1.docentry --reverse charge
left join (select (SUM(ISNULL(TaxSum, 0)) -sum(ISNULL(RvsChrgTax,0))) 'TaxSum',docentry  from rin4 OTHER where  OTHER.Statype not in (-100,-110,-120,-150) group by docentry) as OTHER on OTHER.docentry=i1.docentry --OTHER AND reverse charge
